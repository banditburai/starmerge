...
def test_is_arbitrary_image():
    ...
    assert (
        is_arbitrary_image("[image:not-url]") is True
    )  # Different from JS (would be False)
...
def test_is_arbitrary_size():
    ...
    # Note: Python implementation behaves differently than JavaScript for this case
    assert (
        is_arbitrary_size("[length:12px]") is True
    )  # Different from JS (would be False)
...
def test_is_shadow():
    ...
    assert (
        is_shadow("2px_2px_#000") is True
    )  # Different from comment (is actually True in Python)
    assert (
        is_shadow("2px_2px_black") is True
    )  # Different from comment (is actually True in Python)
